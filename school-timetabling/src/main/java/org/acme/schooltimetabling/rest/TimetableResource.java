...
public class TimetableResource {
    ...
    public String solve(Timetable problem) {
        problem.updateLessonValueRanges();
    ...
    /**
     * Recalculates score and per-lesson violation labels after a manual Demo UI edit
     * (drag-and-drop assignment). Does not start or alter an active solver job.
     */
    @Operation(summary = "Recalculate score and violation labels for a manually edited timetable.")
    @APIResponses(value = {
            @APIResponse(responseCode = "200", description = "Timetable with updated score and violation labels.",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("score")
    public Timetable refreshScore(Timetable problem) {
        problem.updateLessonValueRanges();
        try {
            problem.setScore(solutionManager.update(problem));
        } catch (IllegalStateException e) {
            // Unknown timeslot/room id or other planning-variable state Timefold cannot score.
            throw new TimetableSolverException(null, Response.Status.BAD_REQUEST,
                    "Invalid manual assignment: " + e.getMessage());
        }
        TimetableViolationLabeler.labelViolations(problem);
        return problem;
    }
    ...
    public Timetable getTimeTable(
        ...
        // After solving completes, strip hard-overlap assignments so the UI shows them as unassigned.
